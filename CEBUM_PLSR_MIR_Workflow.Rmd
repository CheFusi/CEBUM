...
calculate_explained_variance <- function(train_data, response_var, max_comps) {
    ...
    # Ensure 'SSN' and the response variable are not included in the predictors (X)
  X <- train_data %>%
    dplyr::select(-c(SSN, all_of(response_var))) %>%
    dplyr::mutate(across(everything(), as.numeric)) %>%
    as.matrix()
  
  # Ensure Y is only the response variable data, converted to a numeric matrix
  Y <- train_data %>%
...
#  print("Structure of predictor matrix X:")
#  str(X)
#  print("Structure of response matrix Y:")
#  str(Y)
  
  # Fit the PLS model for a range of components and store explained variance
  variance_list <- purrr::map_dbl(1:max_comps, function(num_comp) {
    model <- mixOmics::pls(X = X, Y = Y, ncomp = num_comp)
    sum(model$explained_variance$X[1:num_comp])
  })
  
  return(variance_list)
}

fit_model <- function(model_spec_list, train_data, test_data, recipe, model_type, cumulative_variance_threshold = 0.9) {
  model_spec <- model_spec_list$model_spec
  mtry_range <- model_spec_list$mtry_range  # Only for Random Forest
  ...
  if (model_type == "pls") {
    ...
    if (is.na(num_comp) || is.null(num_comp)) {
      warning("No components meet the cumulative variance threshold; setting num_comp to 1 as a default.")
      num_comp <- 1
    }
