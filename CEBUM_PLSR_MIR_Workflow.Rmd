...
calculate_metrics <- function(data, predictions, data_type, slprptr, p, best_params, model_type) {
  ...
  metrics_tibble <- tibble(
    ...
    mtry = if (model_type == "random_forest") best_params$mtry else NA,
    N = nrow(data),
    R2 = r2_value,
    RMSE = rmse_value,
    bias = bias_value
...
# Plot measured vs. predicted values for each model, including validation metrics
plot_predictions <- function(val_plot_data, val_df, slprptr, p, combined_metrics, model_type) {
  ...
  # Save plot
  ggsave(paste0("Plots_Validation_PLSR_MIR_Workflow/", slprptr[p], "_", model_type, ".png"),
...
plot_well_fitted_r2 <- function(all_metrics) {
  ...
  # Identify well-fitted properties based on R² criteria
  well_fitted_properties <- all_r2_values %>%
    filter(R2_diff <= 0.15, R2_cal > 0.6, R2_val > 0.6) %>%
    pull(Property)
  
  # Prepare data for plotting R² values of well-fitted properties
  plot_data <- all_metrics %>%
    ...
    rename(Value = R2)  # Rename R² column to 'Value' for easier plotting
  
  # Generate the plot and store it in a variable
  # Adjust the ggplot code for better label visibility
  well_fitted_plot <- ggplot(plot_data, aes(x = Property, y = Value, fill = Data_Type)) +
    ...
    theme_minimal() +
    theme(
      axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1, size = 8),  # Rotate labels and adjust size
  ...
  ggsave(filename = "well_fitted_properties_plot.png", plot = well_fitted_plot, width = 10, height = 7, dpi = 300)
}

# Wrapper function to run analysis for each property in slprptr
run_all_properties <- function(slprptr, df.f, spec_trt) {
  initialize_workspace()  # Set up directories and metrics tibble
  
  # Loop through each property in slprptr, perform analysis, and collect metrics
  all_metrics <- purrr::map_dfr(92:length(slprptr), function(index) {
    df.sel <- select_property_data(slprptr, index, df.f, spec_trt)  # Filter data for current property
    split <- split_data(df.sel)  # Split data into calibration and validation sets
    cal_df <- split$cal_df
    val_df <- split$val_df
    
    # Check for sufficient data in both calibration and validation sets
    ...
    # Prepare model data and preprocess it
    ...
    # If model_fit indicates "Skipped", skip further steps for this property
    if ("Data_Type" %in% colnames(model_fit) && model_fit$Data_Type[1] == "Skipped") {
      return(model_fit)  # Return skipped information
    }
  ...
  # Summarize results or perform additional analysis on `all_metrics` here
...
# Calculate cumulative explained variance for each component with detailed print statements for troubleshooting
calculate_explained_variance <- function(train_data, response_var, max_comps) {
  ...
  variance_list <- purrr::map_dbl(1:max_comps, function(num_comp) {
    ...
    # Print all attributes of the model to understand its structure
    print("Model prop_expl_var:")
    print(model$prop_expl_var)
    
    # Check the contents of model$variates and model$loadings for variance information
    # print("Model variates (scores):")
    # print(head(model$variates$X))  # Scores for predictors
    # print("Model loadings:")
    # print(head(model$loadings$X))  # Loadings for predictors
    
    # Placeholder if we need to calculate cumulative variance manually
     cumulative_variance <- 0
    # print(paste("Number of components:", num_comp, "- Cumulative variance:", cumulative_variance))
    # cumulative_variance
  })
...
fit_model <- function(model_spec_list, train_data, test_data, recipe, model_type, cumulative_variance_threshold = 0.6) {
  model_spec <- model_spec_list$model_spec
  mtry_range <- model_spec_list$mtry_range  # Only for Random Forest
...
calculate_explained_variance <- function(train_data, response_var, max_comps) {
  ...
  variance_list <- purrr::map_dbl(1:max_comps, function(num_comp) {
    ...
    # Check if prop_expl_var exists and use it to calculate cumulative variance
    if (!is.null(model$prop_expl_var)) {
      cumulative_variance <- sum(model$prop_expl_var$X[1:num_comp])
      print(paste("Number of components:", num_comp, "- Cumulative variance:", cumulative_variance))
      return(cumulative_variance)
    } else {
      # If prop_expl_var is missing, return 0 as a fallback
      print("Warning: prop_expl_var not found in model")
      return(0)
    }
  })
