...
fit_model <- function(model_spec_list, train_data, test_data, recipe, model_type,
  ...
  if (model_type == "pls") {
    ...
    # Random Forest tuning remains unchanged
...
# Run analysis for all properties
identifier <- initialize_workspace("PLSR_RMSEP_sourcesplit_99perc_80_20")  # For RMSEP
run_all_properties(slprptr, df1, spec_trt, model_type = "pls", identifier, 
                   use_source = TRUE, tuning_goal = "rmsep")
```
...
# Run analysis for all properties
identifier <- initialize_workspace("PLSR_RMSEP_CornellOnlysplit_99perc_80_20")  # For RMSEP
run_all_properties(slprptr, df1, spec_trt, model_type = "pls", identifier, 
                   use_source = FALSE, source_name = "Cornell ", tuning_goal = "rmsep")
```
...
# identifier <- initialize_workspace("RF_RMSEP_totsplit_99perc")  # For RMSEP
...
#### Faster computation: reduced resampling (CV 5 fold) and reduced segmenting of mtry and min_n (2)
...
#   cv_folds <- if (nrow(train_data) <= 10) {
#     set.seed(123)  # Set the seed for reproducibility of CV splits
#     vfold_cv(train_data, v = nrow(train_data))  # Leave-One-Out CV if samples are <= 10
#   } else {
#     set.seed(123)  # Set the seed for reproducibility of CV splits
#     vfold_cv(train_data, v = 5)  # 10-fold CV for > 10 samples
#   }
#   
...
#     rf_grid <- dials::grid_regular(mtry_range, dials::min_n(), levels = 3)
#     tune_results <- tune_grid(
#       biochar_workflow,
#       resamples = cv_folds,
#       grid = rf_grid,
...
# identifier <- initialize_workspace("RF_RMSEP_CV5_level2")  # For RMSEP
# run_all_properties(slprptr, df1, spec_trt, model_type = "rf", identifier)
```
...
combined_data_2 <- combined_data %>%
  select(SSN, pH, Primary_Feedstock, Temperature, C_tot_avg, dataset)

# Updated KNN Imputation with multiple predictors
...
trying to fix the issue of working with multiple predictors
...
knn_impute_with_multiple_predictors <- function(data) {
  # Identify rows with missing pH
  missing_idx <- which(is.na(data$pH))
  print(colnames(data)) # Check the column names in the subset
...
imputed_data <- combined_data %>%
 # group_by(Primary_Feedstock) %>%
  group_modify(~ knn_impute_with_multiple_predictors(.x)) %>%
  #group_modify(~ knn_impute_with_multiple_predictors(.x), .keep = TRUE) %>%
  ungroup()
...
# Select only relevant columns for grouping and imputation
combined_data <- combined_data %>%
  dplyr::select(SSN, pH, Primary_Feedstock, C_tot_avg, dataset)
...
knn_impute_with_temp <- function(data) {
  ...
  # Split data into training and testing sets
  train <- data %>% filter(!is.na(pH) & !is.na(C_tot_avg)) # Complete cases only
  test <- data %>% filter(is.na(pH) & !is.na(C_tot_avg))   # Missing pH but with valid Temperature
  ...
  # Apply KNN regression
  knn_result <- knn.reg(
    train = train %>% dplyr::select(C_tot_avg) %>% as.matrix(), # Temperature as predictor
    test = test %>% dplyr::select(C_tot_avg) %>% as.matrix(),  # Temperature as predictor
    y = train$pH,                                         # Non-missing pH values
    k = k
  )
  
  # Impute missing values
  data$pH[is.na(data$pH) & !is.na(data$C_tot_avg)] <- knn_result$pred
  return(data)
}
...
Davis imputed pH - Temp, feedstock, C, Ash
