...
required_packages <- c("writexl","randomForest", "caret", "pls", "data.table", "ithir", 
                       ...
                       "mixOmics", "yardstick", "purrr", "tibble","ranger","VIM","shiny")
...
prepare_model_data <- function(cal_df, val_df, slprptr, .x) {
  if (nrow(cal_df) <= 7 || nrow(val_df) <= 7) {
...
run_all_properties <- function(slprptr, df1, spec_trt, model_type, identifier, 
  ...
  all_metrics <- purrr::map_dfr(2:length(slprptr), function(.x) {
    ...
    if (nrow(data_prep$train_data) <= 7 || nrow(data_prep$test_data) <= 7) {
...
test to catch incorrect number of dimensions error
...
identifier <- initialize_workspace("PLSR_RMSEP_totsplit_99perc")  # For RMSEP
...
# Run analysis for all properties
identifier <- initialize_workspace("PLSR_RMSEP_sourcesplit_99perc")  # For RMSEP
run_all_properties(slprptr, df1, spec_trt, model_type = "pls", identifier, 
...
# Run analysis for all properties
identifier <- initialize_workspace("PLSR_RMSEP_CornellOnlysplit_99perc")  # For RMSEP
run_all_properties(slprptr, df1, spec_trt, model_type = "pls", identifier, 
...
# Run analysis for all properties
identifier <- initialize_workspace("PLSR_Variance_totsplit_99perc")  # For RMSEP
run_all_properties(slprptr, df1, spec_trt, model_type = "pls", identifier, 
...
# Run analysis for all properties
identifier <- initialize_workspace("PLSR_Variance_sourcesplit_99perc")  # For RMSEP
run_all_properties(slprptr, df1, spec_trt, model_type = "pls", identifier, 
...
# Run analysis for all properties
identifier <- initialize_workspace("PLSR_Variance_CornellOnlysplit_99perc")  # For RMSEP
run_all_properties(slprptr, df1, spec_trt, model_type = "pls", identifier, 
...
# Run analysis for all properties
identifier <- initialize_workspace("PLSR_RMSEP_totsplit_99perc_KNN5")  # For RMSEP
